...
public class UserController {
	...
	//to update a user
	@PutMapping("/user/access-id={id}/uid={uid}")
	public ResponseEntity<UserDto> updateUser(@PathVariable Long id, @RequestBody UserDto user, @PathVariable Long uid)
		{
			...
			if(s.equals(s1))
			{
				return new ResponseEntity(userservice.updateUser(user, uid), HttpStatus.OK);
			}
			else if(s.equals(s2))
			{
				return new ResponseEntity(userservice.updateUser(user, uid), HttpStatus.OK);
